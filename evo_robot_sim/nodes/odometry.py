...
import import tf_conversions.posemath as tfc
import tf
...
def callback(jntstate):
    ...
    rotations = dict(zip(jntstate.name, jntstate.position))

    rotations_diffs = {}

    for k in g_joints:
        rotations_diffs[k] = rotations[k] - g_last_rotations[k]
        g_last_rotations[k] = rotations[k]

    # invert left side
    rotations_diffs['fl_lift_link_2_fl_rim_link'] = -rotations_diffs['fl_lift_link_2_fl_rim_link']
    rotations_diffs['bl_lift_link_2_bl_rim_link'] = -rotations_diffs['bl_lift_link_2_bl_rim_link']

    # DAM: stoped here --> tf and geometry_msgs in python is a pita --> will move this and other nodes to gazebo plugin

    print(rotations_diffs)
