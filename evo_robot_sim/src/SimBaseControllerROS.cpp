#include "SimBaseControllerROS.h"
namespace evo {
   ...
   _loop_rate_hz(50)
{
   ...
   privateNh.param("joint_name_back_left", joint_name_back_left, std::string("bl_lift_link_2_bl_rim_link"));
   privateNh.param("joint_name_front_right", joint_name_front_right, std::string("fr_lift_link_2_fr_rim_link"));
   privateNh.param("joint_name_back_right", joint_name_back_right, std::string("br_lift_link_2_br_rim_link"));
...
void SimBaseControllerROS::cbJointState(const sensor_msgs::JointState::ConstPtr& jointstate)
{
   ...
   _mecanum_drive.wheelData2OdomVel(wheel_velocities, odom_vel, -_mecanum_drive.getFacRad2m());

   _mecanum_drive.wheelData2OdomPoseInc(wheel_positions, _last_wheel_positions, odom_pose_increment, -_mecanum_drive.getFacRad2m());
...
void SimBaseControllerROS::checkAndApplyCmdVel()
{
   ...
   if(ros::Time::now().toSec() > (_stamp_cmd_vel.toSec() + _timeout_cmd_vel))
   {
      ...
      MecanumVel zero;
      _mecanum_drive.cmdVel2wheelData(zero, wd, -_mecanum_drive.getFacM2Rad());
   }
   else
   {
      _mecanum_drive.cmdVel2wheelData(_cmd_vel, wd, -_mecanum_drive.getFacM2Rad());
   }
